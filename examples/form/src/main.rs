...
use axum_liveview::{
    bindings::{axm_data, Axm::*, FormEvent},
...
impl LiveView for FormView {
    ...
    fn render(&self) -> Html {
        html! {
            <form { Change }="validate" { Submit }="submit" { Throttle }="1000">
                <label>
                    <div>"Text input"</div>
                    <input type="text" name="input" { Input }="text_input_changed" { Debounce }="1000" />
                ...
                <label>
                    <div>"Textarea"</div>
                    <textarea name="textarea" { Input }="textarea_changed"></textarea>
                    <div>
                ...
                <label>
                    <div>"Select"</div>
                    <select name="number" { Change }="changed" { axm_data("input") }="select">
                        for n in 0..5 {
                ...
                <label>
                    ...
                    <select name="numbers" size="6" multiple { Change }="changed" { axm_data("input") }="multi-select">
                        for n in 0..5 {
                            <option value={ n }>{ n }</option>
                        }
                    </select>
                        ...
                        <div>
                            <label>
                                <input
                                    ...
                                    value={ n }
                                    { Change }="changed"
                                    { axm_data("input") }={ format!("checkbox-{}", n) }
                                />
                                { n }
                            </label>
                        </div>
